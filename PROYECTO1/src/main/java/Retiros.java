            ...
            for (Registro.Cliente cliente : clientes) {
                for (Registro.Cuenta cuenta : cliente.getCuentas()) {
                    if (cuenta.getId().equals(cuentaId)) {
                        ...
                        Registro.Transaccion transaccion = new Registro.Transaccion(idTransaccion,fecha,"Retiro",monto1,0,nuevosaldo);
                        cliente.getTransaccion().add(transaccion);
                        JOptionPane.showMessageDialog( this, "Retiro realizado extisomanete", "Informacion", JOptionPane.INFORMATION_MESSAGE);
                        return;
                    }
                }
            }
